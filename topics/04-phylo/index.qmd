...
rvar_list <- phylo_obs_cen_samp[, "b0_x"]

# ----- build x_avg_post_long -----
posterior_df <- make_rvar_df(phylo_obs_cen_samp)

# select x_avg column
x_avg_col <- posterior_df[, "x_avg", drop = FALSE]
...
sp_id <- rep(seq_len(nrow(x_avg_col)), times = sapply(x_avg_col, length))

# compute x_total_avg
x_total_avg <- rvar_list[sp_id] + x_avg_values

x_avg_post_long <- data.frame(
  sp_id = sp_id,
  x_avg = x_avg_values,
...
merged_df <- merge(obs_xy_df, x_avg_post_long, by = "sp_id", all.x = TRUE)
