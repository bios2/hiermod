...
To practice our first models in Stan, we'll develop an imaginary example based on [yesterday's activity](topics/00_distributions). We'll imagine we're conducting a particularly pleasant study: counting birds! 
...
One of the most useful particularities of Bayesian models is that they are _generative_: they can be used to make a simulated dataset. We'll do that now for our bird example.
...
set.seed(43)
n_people <- 22
avg_birds_per_person <- runif(1, min = 0, max = 18)
...
hist(bird_count,
     col = "lightblue", 
     xlim = range(0,bird_count+5),
     main = "Histogram of bird count")
```

This is pretty great, and represents one possible realization of sampling. 
However, one sample isn't enough to tell us about what our $\text{Uniform}(0, 30)$ prior really means. 
...
This Stan program has two parts. Each part is separated with curly braces, `{}`. There are they `data` block:
...
Let's look at 12 of these datasets and visualize them.
...
brCompare <- seq(round(min(pois_simul$avg_birds_per_person)),
                 round(max(pois_simul$avg_birds_per_person)),
                 length = 60)
