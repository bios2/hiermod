...
all_species_unpooled <- cmdstan_model(
  stan_file = "topics/all_species_unpooled.stan", 
  pedantic = TRUE)
...
post_pred <- spread_rvars(all_species_unpooled_posterior, 
...
```{r}
long_rvars <- gather_rvars(logistic_bern_logit_multispp_samps, 
             intercept[spp_id], slope[spp_id]) 
...
all_species_partpooled_diag <- cmdstan_model(
  stan_file = "topics/all_species_partpooled_diag.stan", 
  pedantic = TRUE)
...
all_species_partpooled_diag_posterior <- all_species_partpooled_diag$sample(data = mite_data_list)
```
