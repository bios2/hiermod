...
From this model which the average weight when a Mallard duck hatches is `r round(coef(reg)[1])` grams (intercept), with an average daily growth of `r round(coef(reg)[2], 1)` grams (intercept).
:::
...
Estimating the intercept and slope parameters of the simple linear regression can be done with an MCMC. This amount to sampling $\beta_0$ as

$$\beta_0 \sim \mathcal{N}(\mu_I, \sigma_I^2)$$
and $\beta$ as 

$$\beta \sim \mathcal{N}(\mu_S, \sigma_S^2)$$
