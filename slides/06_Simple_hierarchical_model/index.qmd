...
$$\mathbf{Z}_{f[i]}$$
This means that, within $\mathbf{Z}$, we focus on the $i^{\text{th}}$ factor $f$. 
...
$$\mathbf{y} \sim \mathcal{MVN}(\mathbf{b}_{f[i]},\sigma^2\mathbf{I}) \quad \forall\quad i = 1\dots n$$
or 
...
$$\mathbf{y} \sim \mathcal{MVN}(\beta_0 + \mathbf{Z}\mathbf{b}_{f[i]},\sigma^2\mathbf{I}) \quad\forall\quad i = 1\dots n$$
or 
...
$$\mathbf{b} \sim \mathcal{N}(0, \sigma^2_f)$$
with the sole difference that $\mathbf{b}$ is linked to an explanatory variable when the hierarchy is on the slope, while when the hierarchy is on the intercept it is not linked to any explanatory variable.
...
::: {style="font-size: 0.7em"}
$$\mathbf{y} \sim \mathcal{MVN}(\boldsymbol{\beta}_{f[i]},\sigma_\mathbf{y}^2\mathbf{I}) \quad \forall\quad i = 1\dots n$$
or 

$$y_i = \beta_{f[i]} + \varepsilon \quad \forall\quad i = 1\dots n$$
In this model, we assume that $\boldsymbol{\beta}_{f[i]}$ is distributed as 

$$\boldsymbol{\beta}_{f[l]} \sim \mathcal{N}(\mu_{f}, \sigma^2_{f})$$
This means that all samples **among** the levels of factor $f$ are used to estimate $\boldsymbol{\beta}_{f[i]}$. 
...
$$\boldsymbol{\beta}_{f[i]} \sim \mathcal{N}(\mu_{f}, \sigma^2_{f})$$
When we study this way of sampling $\boldsymbol{\beta}_{f[i]}$, although our interest is more on $\mu_{f}$, we also have to estimate the variance term $\sigma^2_{f}$.
...
$$\mathbf{y} \sim \mathcal{MVN}(\boldsymbol{\beta}_0+\mathbf{X}\boldsymbol{\beta}_{f[i]},\sigma_\mathbf{y}^2\mathbf{I}) \quad \forall\quad l = 1\dots k$$
or 

$$y_i = \beta_0 + \beta_{f_[i]}x_i + \varepsilon \quad \forall\quad i = 1\dots n$$
In this model, we assume that $\boldsymbol{\beta}_{f[i]}$ is distributed as 

$$\boldsymbol{\beta}_{f[i]} \sim \mathcal{N}(\mu_{f}, \sigma^2_{f})$$
This means that all the samples **among** the levels of factor $f$ are used to estimate $\boldsymbol{\beta}_{f[i]}$. 
