...
::: {style="font-size: 0.7em"}
$$\mathbf{y} \sim \mathcal{MVN}(\boldsymbol{\beta}_{f[l]},\sigma_\mathbf{y}^2\mathbf{I}) \quad \forall\quad l = 1\dots k$$
or 
...
In words, this means that all the samples **among** the $l^\text{th}$ level of factor $f$ are used to estimate $\boldsymbol{\beta}_{f[l]}$. 
...
$$\boldsymbol{\beta}_{f[l]} \sim \mathcal{N}(\mu_{f}, \sigma^2_{f})$$
When we study this way of sampling $\boldsymbol{\beta}_{f[l]}$, although our interest is more on $\mu_{f}$, we also have to estimate the variance term $\sigma^2_{f}$.
...
$$\mathbf{y} \sim \mathcal{MVN}(\boldsymbol{\beta}_0+\mathbf{X}\boldsymbol{\beta}_{f[l]},\sigma_\mathbf{y}^2\mathbf{I}) \quad \forall\quad l = 1\dots k$$
or 
...
the number of levels are not explicitly defined and it is not clear if $ \mathbf{\Sigma}$ includes the same variance value on the diagonal or different ones and whether the off diagonal elements are 0 or another values. 
